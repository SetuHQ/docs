...
import type { EmbeddingConfig, DocumentChunk } from './types.js';
...
async function main(): Promise<void> {
  ...
  if (badHashes.length > 0) {
    for (const c of badHashes.slice(0, 5)) {
      console.error(`  ${c.doc_path}: malformed content_hash "${c.content_hash}"`);
    }
  ...
  if (stats.totalVectorCount > 0) {
    ...
    try {
      ...
      const backupRecords: Array<{ id: string; metadata: Record<string, any> }> = [];
      const BATCH = 100;
      for (let i = 0; i < ids.length; i += BATCH) {
        ...
        if ((i + BATCH) % 500 === 0 || i + BATCH >= ids.length) {
          console.log(`  Backed up ${Math.min(i + BATCH, ids.length)}/${ids.length}...`);
        }
      }
      ...
      console.warn(`  Saving stats-only backup (${stats.totalVectorCount} vectors)`);
      ...
      await fs.writeFile(backupPath, JSON.stringify(statsBackup, null, 2), 'utf-8');
    }
  }
...
function printRestoreInstructions(backupPath: string, indexName: string): void {
  ...
  if (backupPath) {
    ...
    console.error('   It contains vector IDs + metadata from the previous state.');
  } else {
    console.error('1. No backup was created (namespace was empty before this run).');
  }
  console.error('');
  ...
  console.error('   The pipeline is idempotent — existing vectors are skipped.');
  console.error('');
  console.error(`4. To manually inspect the index:`);
  console.error(`     Pinecone index: ${indexName}`);
  console.error('');
  console.error('5. Do NOT manually delete vectors unless you understand the impact.');
  console.error('');
}
