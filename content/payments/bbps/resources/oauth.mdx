...
Setu will make a request to the OAuth Token URL using the credentials configured on Bridge to get the `access_token`. 
This token would then be used to make `/fetch` and `/fetchReceipt` requests to the `BASE URL` configured on Bridge as follows.

Assume `BASE URL` to be `https://base-url.com`

<CodeBlockWithCopy language="shell">
  {`curl --location --request POST 'https://base-url.com/bills/<fetch/fetchReceipt>' \\
...
Refer to the below curl requests and responses based on the chosen `OAuth2.0 scheme` on Bridge
...
<CodeBlockWithCopy language="shell">
  {
    `
    curl --location --request POST 'https://<Token URL>' \\
