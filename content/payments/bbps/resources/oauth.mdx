...
<Callout type="warning">
  Setu currently doesn't consume the <code>refresh_token</code>, but it is accepted if sent back in the response body.
  Setu will make a new token request after the expiry of the previous <code>access_token</code>.
  The default expiry of the <code>access_token</code> is required to be part of the OAuth2.0 configuration on Bridge.
</Callout>
