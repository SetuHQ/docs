...
page_title: UPI Setu - Merchant on-boarding - Registering a VPA
order: 3
  ...
  tabs={[
    {
      ...
      content: (
        <>
          <Text>
            <code>vpa</code> is the only parameter which is required to register
            a VPA for a merchant, if it is available, it will be registered and
            assigned against the merchant whose id is passed in the{" "}
            <code>merchantId</code> header.
          </Text>
