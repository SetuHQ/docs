...
page_title: UPI Setu - Mandates - Execute mandate
order: 3
...
<a href="/payments/umap/mandates/recur/pre-debit-notify" target="_self">
...
<Callout type="tip">
  A merchant needs to manually execute the mandate, <strong>only if</strong>{" "}
...
Once the pre-debit notification against the manadate is `successful`, the merchant can
execute the mandate on the specified `executionDate` while sending the pre-debit
notification.
...
<p>
  <b>Request body parameter values for executing a recurring mandate</b>
</p>
<table>
  <tr>
    <th>parameter</th>
  ...
  <tr>
    <td>amount</td>
    <td>
      must be equal to the <code>amount</code> mentioned while sending mandate
      pre debit notification
    </td>
  </tr>
  <tr>
...
<p>
  ...
  <Link href="/payments/umap/mandates/recur/execute#check-mandate-execution-status">
...
<Callout type="highlight">
  ...
  <code>true</code> while creating a mandate, UPI Setu will schedule executions
  and send notifications to the merchant for <code>execution confirmation</code>{" "}
  and <code>mandate status</code> in every recurrence cycle
</Callout>
...
<p>
  ...
  <a href="/payments/umap/api-reference#/category~mandates/operation~mandates#fetchMandateExecutionById" target="_blank">here</a>
  .
</p>
...
<NextPage
  info={{
    ...
    slug: "/payments/umap/mandates/recur/check-status",
