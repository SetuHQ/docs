...
When a customer initiates pausing a mandate from their UPI app, UMAP will send notifications to the merchant for following events—
- <Text><code>mandate_operation.pause.initiated</code></Text>
- <Text><code>mandate_operation.pause.success</code>, &nbsp; if pause is successful</Text>
- <Text><code>mandate.paused</code>, &nbsp; terminal status, if pause is successful</Text>
- <Text><code>mandate_operation.pause.failed</code>, &nbsp; if pause fails</Text>
- <Text><code>mandate.live</code>, &nbsp; terminal status, if pause fails</Text>
...
<Callout type="warning">
    The only operation that can be performed on a <strong>paused</strong> mandate is to unpause it. Other operations like{" "}
