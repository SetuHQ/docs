...
<Card padding="nano" shape="rounded" marginBottom="micro" marginTop="micro">
    ...
    <a href="/payments/umap/mandates/otm/create">Create one time mandate ↗</a>
</Card>
...
<table>
    ...
    <tr>
        <td>amount</td>
        <td>
            must be as per the <code>amountRule</code> and{" "}
            <code>maxAmountLimit</code> configured while creating the mandate
        </td>
    </tr>
    <tr>
        <td>executionDate</td>
...
<ul>
    <li>
        <b>mandate_operation.notify.success</b> (if mandate pre debit
        notification is successful)
    </li>
    <li>
        <b>mandate_operation.notify.failed</b> (if mandate pre debit
        notification fails)
    </li>
</ul>
<p>
...
<p>
    ...
    <Link href="/payments/umap/mandates/otm/pre-debit-notify#check-mandate-pre-debit-notification-status">
