...
<p>In addition to the notifications, the merchant can also use <Link href="/payments/umap/mandates/otm/pre-debit-notify#check-mandate-pre-debit-notification-status">check mandate pre debit notification status API</Link> to check if pre debit notification is <b>successful</b></p>
