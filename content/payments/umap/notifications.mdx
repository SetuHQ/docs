...
A Merchant can recieve status updates by registering a callback URL and subscribing certain events under it. Though merchants can configure any number of callback URLs, an event can be subscribed to only once at a mechant level.
