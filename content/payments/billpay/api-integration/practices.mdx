...
Ideally a transaction has a duration of 180 seconds for completion so the status will be notified via webhooks within 180 seconds. Even for pending transactions, the response will be posted via webhook as soon as the status changes. Post that polling `/response` API once in 15 minutes can be tried. But we strongly recommended looking out for the webhook notifications.
...
```js
{
    "data": {
        "failureReason": {
            "code": "BOU002",    // Payment failed due to type "BBPS". Fetch can be reused in the next 48 hours
            "message": "TMM001", // from the first try of the Payment. In case the Biller is of Validation Type,
            "type": "BBPS"       // Ageing period will be 24 hours of successful validation.
        },
        "refId": "CE9U2F9RHQPJICM8SC0G87SZTUR23440700",
        "refundDetails": {
            "status": "Success",
...
<details>
  ...
  ```js
{
    "refId": "CE9U2F9RHQPJICM8SC0G87SZTUR23440700", // Provided refId in the Payment Request is the one linked to the Bill fetch (for which the Bill payment was a failure with "BBPS" error).
    "paymentDetails": {
        "mode": "Credit Card",
        "paymentRefId": "N2001121212344",
        "amount": 100000,
...
{
    "data": {
        "refId": "CE9U4I9RHQPJICM8SCK0303EZUD23440705" // A different refID is returned in Payment Response.
    },
    "success": true,
...
<details>
  ...
  ```js
{
    "refId": "CE9U4I9RHQPJICM8SCK0303EZUD23440705" // Using the new refId, we can find the status of the second transaction.
}
  ```
