...
The request api has been updated to include the new dispute types <a href="/payments/billpay/api-integration/api-reference#/category~Dispute/path~/api/v2/bbps/bills/complaint/request/post"> See API Reference </a>.
...
The response api has been updated to include the new status values and fields <a href="/payments/billpay/api-integration/api-reference#/category~Dispute/path~/api/v2/bbps/bills/complaint/response/post"> See API Reference </a>.
...
| `FAILED` | The dispute could not be raised with NPCI due to technical or network issues |
...
2. **Webhook updates**: All <a href="/payments/billpay/api-integration/webhooks#dispute-webhook" target="_blank">webhook/callback responses</a> now include the new fields
