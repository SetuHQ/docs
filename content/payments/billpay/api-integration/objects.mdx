...
CustomerV2 object holds the customer mobile number and the customer bill parameters. This is a new object introduced in the V2 API Spec.
...
## Biller (List billers response)

The List billers API returns a richer biller object. For **FX Retail** flows, the following fields are relevant:

| parameter | type | description |
| --- | --- | --- |
| `valAddFlag` | `boolean` | Indicates if value-added services are enabled for the biller. |
| `valAddCustParams` | `object` | Customer parameters required for value-added service flows. Each entry is scoped by `requestType`. For FX Retail billers, `requestType` can be `MANDATE_AND_PAY`. |
| `mandateRequirement` | `string` | Mandate requirement for the biller (example: `MANDATORY`). For FX Retail billers, use this to determine if the biller requires the mandate+pay flow. |
...
| accountInfo            | `string` (Mandatory)                                    | NPCI-compatible instrument info for the payment instrument. |
| custConvFee            | `integer` (Optional)                                    | Customer convenience fee in paise (when applicable).        |
| Id                     | `string` (Optional)                                     | Plan ID for plan-based billers.                             |
