...
This section introduces the core concepts of UPMS, its benefits, and the terminology an AI will encounter while integrating.
...
Think of it as a **subscription service for bills**. Instead of end-users manually fetching their bill details (like amount and due date) each cycle for various billers (electricity, phone, loan EMIs, etc.), UPMS allows them to register once through any AI application.
...
2. BBPS makes this information available to Setu.
3. We then notify AI application about the new bill via a webhook (callback).

This enables a seamless, automated bill presentment experience, right within an AI application.

### 1.2 Why Integrate with UPMS?

Integrating with UPMS offers significant advantages for both an AI application and the end-users:

- **Improved End-User Experience**: Registered users see their bills automatically in the AI application, removing the need for them or the AI to fetch bills each cycle. This shifts bill management from a manual "pull" to an automated "push" model.
- **Timely Bill Awareness & Centralized View**: Proactive notifications about new bills and due dates help users avoid missed payments and late fees. Users can manage subscribed bills from different billers in one place— the AI application.
- **Increased Engagement**: Simplified bill discovery encourages users to pay digitally via the AI application, boosting engagement.

### 1.3 How UPMS Works

A simplified UPMS flow when using Setu APIs:

1. **User Action**: An end-user on AI application requests to register for automatic bill presentment for a specific biller (e.g., electricity provider).
2. **API Call (AI → Setu)**: AI app calls Setu **Fetch Bill and Create Registration** API with customer and mandate details.
3. **BBPS Interaction (Setu → BBPS → Biller)**: We interact securely with BBPS central unit and the biller's system to validate and record the registration.
4. **Acknowledgement (Setu → AI)**: Setu's API returns a synchronous response (typically `PENDING`).
5. **Asynchronous Confirmation (Setu → AI)**: Final registration status (`SUCCESS` or `FAILED`) is communicated later via webhook.
6. **Bill Push (Biller → BBPS → Setu)**: On a new bill cycle, the biller pushes bill details to BBPS.
7. **Bill Notification (Setu → AI)**: We send a callback to your configured endpoint with the bill details.
8. **Presentment (AI App)**: AI app parses the callback, identifies the user, and displays the bill.
...
A UPMS registration represents an end-customer's mandate with a UPMS enabled biller for automatic bill presentment via AI application. A successful registration authorizes the biller (with customer consent) to push bill details for that customer through BBPS to Setu, and subsequently, to AI via callbacks.

<Callout type="warning">
<b>UPMS is only available for billers who have the <code>upmsEnabled</code> flag set to <code>true</code>.</b> AIs can check this flag using the <a href="/payments/billpay/api-integration/api-reference#/category~List/operation~getBillers">List billers API</a>, which also allows filtering by this flag. Trying to create a UPMS registration for billers without <code>upmsEnabled</code> set to <code>true</code> will fail.
</Callout>
...
- **VIEW_N_PAY**: Standard presentment; AI application receives bill notifications, displays them, and the user manually initiates payment.
- **AUTO_PAY**: Auto-payment mandate already set up. When a bill is presented, using Setu’s payments APIs, the payment is initiated from the AI’s current account via BBPS.
...
Additionally, an AI will receive a callback for each bill generated for the registered user.

Ensure an AI implements a reliable webhook listener to receive and process these callbacks.

# Part 2: Managing Customer Registrations

## Prerequisite: Configure Your Registration Callback URL

AI must set up and provide Setu with a secure HTTPS webhook endpoint URL. This endpoint is crucial as it's where Setu system will send asynchronous notifications (via HTTP POST) regarding:
...
This process leverages the existing **Bill Fetch API** endpoint. To initiate a UPMS registration alongside fetching the current bill (if available), AI simply include the `mandate` object in the standard Bill Fetch request.
        ...
        <CodeBlockWithCopy language="json">{`
{
  ...
  "data": {
    ...
    "upmsRegistration": {
      // Status of the registration request processing by Setu system
      "status": "PENDING", 
...
- **Bill Fetch Callback (If Bill Found):** If a current bill exists for the customer, the AI will receive a `BILL_FETCH` callback containing the bill details. Handle this as described in Part 3. This callback is independent of the registration success/failure.

- **Registration Status Callback:** For registrations with initial status as `PENDING`, the AI will receive a `CREATE_UPMS_REGISTRATION` callback to inform AI of the final outcome of the registration attempt identified by `upmsRegistration.refId` from the synchronous response.
...
This synchronous endpoint allows AIs to retrieve the current status and details of a specific, existing UPMS registration immediately.
...
- **Path Parameter:**
  - `upmsRegistrationRefID` (string): The unique reference ID of the UPMS registration the AI wants to view (obtained from a successful `CREATE_UPMS_REGISTRATION` callback, e.g., `D07LJTB71N0PGH5KG32G6dH2QjK51181605`).
...
AI will receive an `UPDATE_UPMS_REGISTRATION` callback with the final outcome.
...
AI will receive a `CANCEL_UPMS_REGISTRATION` callback confirming the outcome.
...
Once a customer is successfully registered for UPMS (see Part 2), the core value proposition comes into play: receiving automatic bill presentments. This part guides AIs on how to handle these presented bills and integrate them into your payment flows.

## Prerequisite: Configure Your Skip Payment Callback URL

AI must set up and provide Setu with a secure HTTPS webhook endpoint URL. This endpoint is crucial as it's where Setu system will send asynchronous notifications (via HTTP POST) regarding presented bills and skipped payments (`BILL_FETCH`, `SKIP_PAYMENT` events).

## 4. Receiving Presented Bills

The primary way an AI receives details about a newly generated bill for a registered customer is through the standard **Bill Notification callback** (`BILL_FETCH` event). This is the same event used for responses to manual bill fetches, but includes an identifier for UPMS.
...
3. **Notification to COU**: BBPS identifies the UPMS registration and notifies Setu.
4. **Callback to AI**: Setu sends a `BILL_FETCH` webhook with the bill details. (<a href="/payments/billpay/api-integration/webhooks#bill-fetch-webhook">See Webhook Reference</a>)
5. **Your Processing**:
   ...
   - Display bill details in your application.
...
> **Note**: The same webhook endpoint receives notifications for both automatically presented bills (via UPMS) and bills retrieved through manual fetch requests (using separate Bill Fetch API). AI can distinguish between these two scenarios by checking the `upmsRegistrationRefId` field within the callback's data object:
...
1. **Invoke Payment API**: Construct and send a request to Setu BBPS Payment API endpoint. (<a href="/payments/billpay/api-integration/api-reference#/category~Pay/operation~PaymentRequestV2">See API Reference</a>)
...
Sometimes, a user might pay a bill through a different channel (e.g., directly on the biller's website, via another app) after it has been presented to them in your application via UPMS. To avoid showing already-paid bills as pending, Setu provides a mechanism to notify AIs about such payments.

### 6.1 SKIP_PAYMENT Callback Flow

If a bill is paid elsewhere after presentation, AI will receive a `SKIP_PAYMENT` callback: <a href="/payments/billpay/api-integration/webhooks#skip-payment-webhook">(See Webhook Reference)</a>

1. **Payment Elsewhere**: User pays via another BBPS channel.
2. **Notification to Setu**: BBPS detects external payment and informs Setu.
3. **Callback to AI**: We send a `SKIP_PAYMENT` webhook to your configured endpoint.
4. **Your Processing**:
   - Acknowledge with HTTP `200 OK`.
...
Handling SKIP_PAYMENT callbacks ensures data consistency and improves user experience:
  ...
  - **Accurate Bill Status**: Prevents users from seeing bills they've already paid elsewhere as still outstanding in AI application.
...
This part covers the specialized APIs designed to help AI test their integration, particularly their handling of asynchronous callbacks, in non-production environments.
...
To facilitate this, Setu provides Simulation APIs that allow AI to manually trigger specific callback events on demand.
...
This asynchronous API allows AIs to manually trigger a `BILL_FETCH` callback for a specific, active UPMS registration.
...
- **Headers:**
  ...
  - `Content-Type: application/json`
- **Path Parameter:**
  - `upmsRegistrationRefID` (string): The unique reference ID of the active UPMS registration for which an AI wants to simulate a bill notification. This registration must exist and be in a state like `SUCCESS`.
- **Body:** None
...
- **Callback Payload:** The payload structure will follow the schema described for `BILL_FETCH` events (see Part 3, Section 4), including the `upmsRegistrationRefId` AI specified in the simulation request.

**Action:** AIs to use this API in their test environment to verify that their webhook endpoint correctly receives, parses, and processes incoming bill notifications. Ensure the AI can link the received bill to the correct user based on the `upmsRegistrationRefId`.

## 9. Simulating a Skip Payment Callback

This asynchronous API allows an AI to manually trigger a `SKIP_PAYMENT` callback for a specific bill reference ID.
...
- **Headers:**
  ...
  - `Content-Type: application/json`
- **Path Parameter:**
  - `billRefId` (string): The unique reference ID of the specific bill instance for which the AI wants to simulate a skip payment notification. This `billRefId` would typically have been received previously in a `BILL_FETCH` callback (either real or simulated).
- **Body:** None
...
- **Callback Payload:** The payload structure will follow the schema described for `SKIP_PAYMENT` events (see Part 3, Section 6.1). It will include `event: "SKIP_PAYMENT"`, `timeStamp`, `traceId`, and a `data` object containing the `billRefId` the AI specified and a `dateOfPayment`.

**Action:** AIs to use this API in their test environment to verify that their webhook endpoint correctly receives `SKIP_PAYMENT` events and that their application logic correctly identifies the bill using `billRefId` and updates its status appropriately (e.g., marks it as paid, removes it from pending lists).

## 10. General Testing Recommendations

- **Webhook Listener:** Ensure an AI has a stable and publicly accessible webhook listener endpoint configured in Setu system for your Sandbox/UAT environment before using the simulation APIs.
