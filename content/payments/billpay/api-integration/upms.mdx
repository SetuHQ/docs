...
- **DEACTIVATED**: Registration is inactive (temporarily).
...
This process leverages the existing **Bill Fetch API** endpoint (`POST /bbps/bills/fetch/request`). To initiate a UPMS registration alongside fetching the current bill (if available), you simply include the `mandate` object in the standard Bill Fetch request.

**Endpoint:** `POST /bbps/bills/fetch/request`
...
**Endpoint:** `GET /upms/registrations/{upmsRegistrationRefID}`
...
**Endpoint:** `PATCH /upms/registrations/{upmsRegistrationRefID}`
...
**Endpoint:** `DELETE /upms/registrations/{upmsRegistrationRefID}`
...
**Endpoint:** `POST /upms/simulate/{upmsRegistrationRefID}/callbacks/bill`
...
**Endpoint:** `POST /upms/simulate/{billRefId}/callbacks/skip-payment`
