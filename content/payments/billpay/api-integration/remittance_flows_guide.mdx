...
- **NPCI Compatibility**: The `accountInfo` field must be formatted according to [NPCI specifications](#npci-specification-compliance) and to be passed with all Bill Payments and the `remitter` object to be passed with all Credit Card Bill Payments.
