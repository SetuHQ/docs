...
This guide provides comprehensive specifications for remitter details and account information formatting in bill payments. Remitter details are part of the overall payment integration flow.

📖 **For complete payment integration context and examples → [Paying Bills Integration Guide](./paying_bills.mdx)**

NPCI has introduced comprehensive remittance information handling to accommodate acceptance of remittance data for all bill payments. 
...
|-------|------|-------------|---------|-------|
| `accountInfo` | String | Format varies by payment mode | Contains customer's remittance information in NPCI-compatible format | Required field |
...
| Property | Type | Constraints | Purpose | Notes |
|----------|------|-------------|---------|-------|
| `name` | String | min: 1, max: 50 | Remitter's full name | Auto-fallback: 'Not Available' if not provided |
