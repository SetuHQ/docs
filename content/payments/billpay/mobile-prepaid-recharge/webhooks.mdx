...
<CodeBlockWithCopy language="json">{`{
  ...
  "provider": "string",
  "failureReason": "string (optional, only present for failures/reversals)",
...
|-------|------|-------------|
| `transactionRefId` | string | Transaction reference ID from the original recharge request |
| `trace_id` | string | System reference number (internal trace ID) |
| `operatorRefId` | string | Operator transaction ID (if available) |
| `status` | string | Transaction status - one of: `"Successful"`, `"Processing"`, `"Failure"` |
| `amount` | string | Recharge amount |
| `mobile_number` | string | Mobile number that was recharged |
| `provider` | string | Service provider name |
| `failureReason` | string (optional) | Reason for failure (only present when status is "Failure") |
...
<CodeBlockWithCopy language="json">{`{
  ...
  "balance": "string",
  "message": "string",
  "timestamp": "ISO 8601 datetime string (ending with Z)",
  "traceId": "string"
}`}</CodeBlockWithCopy>
...
| Field | Type | Description |
|-------|------|-------------|
| `event` | string | Always `"wallet.low_balance"` for this webhook type |
| `balance` | string | Current wallet balance amount |
| `message` | string | Alert message (typically "Low balance Alert") |
| `timestamp` | string | ISO 8601 formatted timestamp ending with "Z" (UTC) |
| `traceId` | string | Unique trace ID for this notification |
...
<CodeBlockWithCopy language="json">{`{
  ...
  "creditedFrom": "string",
  "transactionDate": "ISO 8601 datetime string (ending with Z)",
  "message": "string",
  "traceId": "string",
...
| Field | Type | Description |
|-------|------|-------------|
| `event` | string | Always `"wallet.credit"` for this webhook type |
| `amount` | string | Top-up amount credited to wallet |
| `referenceNumber` | string | User reference number for the top-up transaction (may be empty string) |
| `creditedFrom` | string | Source of the credit (e.g., "Bank Transfer", may be empty string) |
| `transactionDate` | string | ISO 8601 formatted timestamp of the transaction (ending with "Z") |
| `message` | string | Transaction message (defaults to "Credit Successful" if not provided) |
| `traceId` | string | Unique trace ID for this notification |
| `currentBalance` | string | Current wallet balance after the top-up |
