...
- **Template**— A template is a pre-configured format, that outlines the structure and organization of the generated report, including which insights to generate. You can pick and choose exactly what modules should be added to a template, based on your needs. For example, if you only want to generate income and expense insights of a customer, the template can contain only those two modules.
...
You need to send in the below information to be configured to [insights@setu.co](mailto:insights@setu.co)
...
| **`Insights required`** | A list of required <a href="/data/insights/postman" target="_blank">**Insights**</a> |
...
<Row>
  ...
  <Portion desktopSpan="half">
    <Card padding="small" shape="rounded">
      ...
      <a
        href="/data/account-aggregator/quickstart/api-integration"
        target="_blank"
      >
