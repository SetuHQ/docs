...
page_title: Penny drop + PennyLess Notifications
order: 2
...
Here’s a sample notification of a successful Penny drop + PennyLess request, with the corresponding request `id` as well as the details of the account holder.
...
Here is a sample notification of a failed Penny drop + PennyLess request, with the corresponding request `id` as well as the reason for failure.
