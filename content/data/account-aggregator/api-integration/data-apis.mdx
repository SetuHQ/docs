...
- Once the combined data session status is `PARTIAL` or `COMPLETED`, Setu fetches the FI data and sends it to the FIU.
  - For one time consents, Auto-Fetch data executes only once, right after the consent is approved.
  - For recurring consents, Auto-Fetch data can fetch data periodically. Set up the frequency of Auto-Fetch data in your configuration on the Bridge.
