      ...
      "accounts": [
        {
          "linkRefNumber": "b2329f47-0a6f-4131-adb5-9ef7b4c1ca6a",
          "maskedAccNumber": "XXXXXX4373",
          "status": "FAILED",  // individual account FI status
          "data": null
        }
      ],
      "fipID": "ACME-FIP"
...
Auto data fetch feature aims to absorb this complexity from the FIUs. With Auto data fetch feature enabled, the process will look like this—

- Consent is approved by the customer.
- Setu creates a data session automatically.
- Setu listens to FI notifications and manages retries.
- Once the combined data session status is `PARTIAL` or `COMPLETED`, Setu fetches the FI data and sends it to the FIU.
  - For one time consents, Auto data fetch executes done only once, right after the consent is approved.
  - For recurring consents, Auto data fetch can fetch data periodically. Set up the frequency of Auto data fetch in your configuration on the Bridge.
