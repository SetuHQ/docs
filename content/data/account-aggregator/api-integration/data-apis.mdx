...
Auto data fetch feature aims to absorb this complexity from the FIUs. With Auto data fetch feature is enabled, the process will look like this—
...
- Once the combined data session status is `READY` or `PARTIAL`, Setu fetches the FI data and sends it to the FIU.
