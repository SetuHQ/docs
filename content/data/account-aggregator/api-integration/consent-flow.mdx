...
1.  **Create consent request** — This is essentially a one-time data request that your user needs to approve. To know more details about this please visit [Consent Object](/data/account-aggregator/consent-object) page. Once a consent is created, you will receive a unique `id` with `status = PENDING`, along with a `url` where you can **redirect your customer to consent manager screens**, to review the data request. Here, the user may approve or reject the request
...
1. **Dual consents** — Merge two consents into a single UI flow, with its own unique dual consent ID.
...
- Create dual consent — Use the provided API endpoint to create a dual consent process by specifying which consents are optional and which are mandatory.
