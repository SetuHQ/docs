...
By default, webhooks deliver status only. Fetch configured outputs (JSON, Excel, XML, and so on) using the Signal IQ APIs. See the <a href="/data/signal-iq/output-apis" target="_blank">API reference</a>. If you want results in the `INSIGHTS_CREATION_SUCCESSFUL` webhook, that can be customised during onboarding - see the <a href="/data/signal-iq/overview#the-experience-is-the-same-across-every-flow" target="_blank">Overview</a>. Notification schemas match the <a href="/data/signal-iq/pdf-flow#notifications-you-receive" target="_blank">PDF Flow notifications</a>.
